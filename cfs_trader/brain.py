...
def _ask(system, user, model="sonnet", timeout=60):
    """claude CLI headless → parse edilmiş JSON dict. Hata FIRLATIR (çağıran try/except ile sarar)."""
    prompt = system + "\n\n" + user
    proc = subprocess.run(
...
def risk_state(cfg, store):
    ...
    if n >= 2 and (longs == 0 or shorts == 0):
        flags.append("yön-konsantrasyonu: %d pozisyonun hepsi %s (korelasyon riski)" % (n, "LONG" if longs else "SHORT"))
    if total_risk > budget * risk_frac:
        flags.append("toplam açık risk %.1f USDT > bütçe×%.1f (%.1f)" % (total_risk, risk_frac, budget * risk_frac))
    if consec >= max_consec - 1:
        flags.append("ardışık zarara yakın: %d / %d (kill-switch eşiği)" % (consec, max_consec))
...
def risk_guardian(cfg, store, notifier=None, model=None, log=print):
    ...
    return {"state": state, "flags": flags, "warn": warn, "severity": sev, "notified": True}
...
{"summary":"<2-3 cümle: bugün ne oldu, genel gidişat>",
 ...
 "actions":["<somut öneri: 'X kaynağını kıs', 'Y eşiğini artır' vb.>"],
...
def _render_daily(today, closed, daily, learning, guard):
    ...
    L.append("\nGörev: yukarıdaki veriye dayanarak gün sonu raporu JSON'u ver.")
    return "\n".join(L)
...
def daily_review(cfg, store, notifier=None, model=None, log=print):
    ...
    if notifier:
        ...
        notifier.send("\n".join(parts))
    return {"summary": summary, "working": working, "bleeding": bleeding, "actions": actions, "guard": guard}
...
def main():
    ...
    ap.add_argument("cmd", choices=["daily", "guardian", "test"], help="daily=gün sonu rapor, guardian=risk gözcü, test=bağlantı testi")
    a = ap.parse_args()
    from .cfg import get
